...
// ── BARU: ambil/buat visitorId (untuk impression tracking) ────
    ...
    <div className="relative w-full overflow-hidden rounded-xl shadow-lg" style={{ height: 420 }}>
      {articles.map((a, i) => (
        <Link key={a.id} href={`/berita/${a.slug}`}>
          <div className="absolute inset-0 transition-opacity duration-700" style={{ opacity: i === current ? 1 : 0, zIndex: i === current ? 1 : 0 }}>
            <Image src={getArticleImage(a)} alt={a.title} fill className="object-cover" priority={i === 0} />
            <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/30 to-transparent" />
            <div className="absolute bottom-0 left-0 right-0 p-6 z-10">
              <span className="inline-block bg-red-700 text-white text-xs font-bold px-3 py-1 rounded mb-2 uppercase tracking-widest">{a.category}</span>
              <h2 className="text-white text-2xl font-bold leading-snug line-clamp-2 mb-2 drop-shadow">{a.title}</h2>
              <p className="text-gray-300 text-sm line-clamp-2 max-w-2xl">{a.excerpt || a.body}</p>
              <div className="flex items-center gap-3 mt-3 text-xs text-gray-400">
                <span>{a.author.name}</span><span>•</span>
                <span>{a.publishedAt ? formatDate(a.publishedAt) : formatDate(a.createdAt)}</span>
                <span>•</span><span>👁 {a.views.toLocaleString()}</span>
              </div>
            </div>
          </div>
        </Link>
      ))}
      <button onClick={() => { setCurrent((c) => (c - 1 + articles.length) % articles.length); resetTimer(); }}
        className="absolute left-3 top-1/2 -translate-y-1/2 z-20 bg-white/80 hover:bg-white text-gray-800 rounded-full w-9 h-9 flex items-center justify-center shadow transition" aria-label="Previous">‹</button>
      <button onClick={() => { setCurrent((c) => (c + 1) % articles.length); resetTimer(); }}
        className="absolute right-3 top-1/2 -translate-y-1/2 z-20 bg-white/80 hover:bg-white text-gray-800 rounded-full w-9 h-9 flex items-center justify-center shadow transition" aria-label="Next">›</button>
      ...
      <Link href={`/berita/${article.slug}`}>
        <article className="flex gap-4 py-4 group hover:bg-gray-50 -mx-1 px-1 rounded-lg transition-colors">
          <div className="relative w-28 h-20 flex-shrink-0 rounded-lg overflow-hidden bg-gray-100">
            <Image src={getArticleImage(article)} alt={article.title} fill className="object-cover group-hover:scale-105 transition-transform duration-300" />
          </div>
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-1.5 mb-1.5">
              <span className={`text-[11px] font-bold uppercase tracking-widest ${colorClass}`}>{article.category}</span>
              <span className="text-gray-300 text-[11px]">•</span>
              <span className="text-[11px] text-gray-400">{dateStr}</span>
            </div>
            <h3 className="text-sm font-bold text-gray-900 leading-snug line-clamp-2 group-hover:text-red-700 transition-colors mb-1.5">{article.title}</h3>
            <p className="text-xs text-gray-500 leading-relaxed line-clamp-2">{article.excerpt || article.body}</p>
          </div>
        </article>
      </Link>
...
// ── BARU: ImpressionWrapper — IntersectionObserver per artikel ─
// Komponen invisible yang memantau kapan artikel masuk viewport
// dan melaporkan satu impression ke API
function ImpressionWrapper({ articleId, children }: { articleId: string; children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      ...
      { threshold: 0.4 } // artikel 40% terlihat = impression
    );
...
export default function Home() {
  ...
  useEffect(() => {
    Promise.all([fetchArticles(), fetchContributors()]).then(([arts, contribs]) => {
      ...
      setLoading(false);
      // Impression tracking untuk carousel (artikel paling atas = selalu terlihat)
      // ImpressionWrapper per artikel akan mengurus sisanya via IntersectionObserver
    });
  }, []);
      ...
      <header className="bg-white border-b border-gray-100 sticky top-0 z-50 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center gap-6">
          <Link href="/" className="shrink-0">
            <Image src="/assets/NarasiKotaLogoBiru.webp" alt="NarasiKota" width={220} height={64} className="h-16 w-auto object-contain" priority />
          </Link>
          <div className="flex-1 max-w-xl">
            <div className="relative">
              <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-800 text-sm">🔍</span>
              <input type="text" placeholder="Pencarian berita..." value={searchQuery}
          ...
          <div className="ml-auto flex items-center gap-3 shrink-0">
            <Link href="/login" className="px-5 py-2 border border-gray-200 text-gray-700 rounded-lg font-semibold text-sm shadow-sm hover:shadow-md hover:border-gray-300 hover:bg-gray-50 active:scale-95 transition-all duration-200">MASUK</Link>
            <Link href="/register" className="px-5 py-2 bg-red-700 text-white rounded-lg font-semibold text-sm shadow-sm hover:shadow-md hover:bg-red-800 active:scale-95 transition-all duration-200">JOIN KONTRIBUTOR</Link>
          </div>
        </div>
      </header>
        ...
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex overflow-x-auto">
            {["Breaking News", "Pasar Modal", "Emiten", "Analisis", "Opini", "Infografis", "Video"].map((item) => (
              <Link key={item} href="#" className="px-5 py-2.5 text-xs font-semibold text-gray-500 whitespace-nowrap hover:text-red-700 transition border-r border-gray-100">{item}</Link>
            ))}
          </div>
        </div>
      </div>
        ...
        {loading ? (
          ...
          <>
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              {/* ── LEFT: Carousel + grid + list ─────────────── */}
              <div className="lg:col-span-2 space-y-0">

                {/* Hero Carousel — artikel carousel = impression via wrapper */}
                {carouselArticles.map((a) => (
                  <ImpressionWrapper key={a.id} articleId={a.id}>
                    <span className="hidden" /> {/* wrapper hanya untuk tracking */}
                  </ImpressionWrapper>
                ))}
                      ...
                      <ImpressionWrapper key={article.id} articleId={article.id}>
                        <Link href={`/berita/${article.slug}`}>
                          <article className="bg-white rounded-lg shadow-sm overflow-hidden group hover:shadow-md transition h-full border border-gray-100">
                            ...
                            <div className="p-2.5">
                              <span className={`text-[10px] font-bold ${categoryColor[article.category] ?? "text-gray-600"}`}>{article.category}</span>
                              <h4 className="text-xs font-semibold text-gray-800 mt-0.5 line-clamp-2 group-hover:text-red-600 transition leading-snug">{article.title}</h4>
                            </div>
                          </article>
                        </Link>
                      </ImpressionWrapper>
                ...
                {/* BERITA TERKINI — dengan ImpressionWrapper per artikel */}
                  ...
                  <div className="bg-white rounded-xl border border-gray-100 shadow-sm mt-4">
                    <div className="flex items-center justify-between px-5 py-3 border-b border-gray-100">
                      <div className="flex items-center gap-2.5">
                        ...
                        <h2 className="text-sm font-bold text-gray-900 uppercase tracking-wide">Berita Terbaru Lainnya</h2>
                      </div>
                      <Link href="#" className="text-xs font-semibold text-blue-600 hover:text-red-700 transition-colors flex items-center gap-1">Indeks berita »</Link>
                    </div>
                    <div className="px-5 divide-y divide-gray-100">
                      {latestArticles.map((article) => (
                        <ImpressionWrapper key={article.id} articleId={article.id}>
                          <KatadataNewsItem article={article} showDivider={false} />
                        </ImpressionWrapper>
                      ))}
                    </div>
                  </div>
              ...
              <div className="space-y-5">
                      ...
                      <ImpressionWrapper key={article.id} articleId={article.id}>
                        <Link href={`/berita/${article.slug}`} className="flex gap-3 p-3 hover:bg-gray-50 transition group">
                          ...
                          <div>
                            ...
                            <p className="text-[10px] text-gray-400 mt-1">{article.publishedAt ? formatDate(article.publishedAt) : formatDate(article.createdAt)}</p>
                          </div>
                        </Link>
                      </ImpressionWrapper>
                    ))}
                  ...
                  <div className="divide-y divide-gray-50">
                    {contributors.length > 0 ? contributors.map((user) => (
                      <div key={user.id} className="flex items-center gap-3 px-4 py-3 hover:bg-gray-50 transition">
                        <div className="w-9 h-9 bg-gradient-to-br from-red-600 to-orange-500 rounded-full flex items-center justify-center text-white font-bold text-sm shrink-0">{user.name.charAt(0)}</div>
                        <div>
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
                  ...
                  <p className="text-xs text-gray-500 mb-3 leading-relaxed">Dapatkan berita terkini langsung ke email Anda setiap hari</p>
                  ...
                  <button className="w-full bg-red-700 hover:bg-red-800 text-white font-bold py-2.5 rounded-lg text-sm transition">Berlangganan Gratis</button>
                </div>
              </div>
            </div>
          </>
      ...
      <footer className="bg-[#f4f4f5] border-t border-gray-200 mt-16">
        <div className="max-w-7xl mx-auto px-4 py-12">
          <div className="grid grid-cols-2 md:grid-cols-5 gap-8">
            <div className="col-span-2 md:col-span-1">
              <Link href="/" className="block mb-4">
                <Image src="/assets/NarasiKotaLogoBiru.webp" alt="NarasiKota" width={260} height={60} className="h-18 w-auto object-contain" />
              </Link>
              <p className="text-xs text-gray-500 leading-relaxed mb-4">Portal berita terpercaya untuk informasi ekonomi, bisnis, dan keuangan Indonesia.</p>
              <p className="text-xs text-gray-400">part of <span className="font-bold text-gray-600">Narasi Kota Network</span></p>
              <div className="flex gap-2 mt-4">
                {["f", "𝕏", "in"].map((icon) => (
                  <a key={icon} href="#" className="w-8 h-8 rounded-full bg-gray-200 hover:bg-red-700 hover:text-white text-gray-600 flex items-center justify-center text-xs font-bold transition">{icon}</a>
                ))}
              </div>
            </div>
            <div>
              <h4 className="font-bold text-gray-800 mb-4 text-sm">Kategori</h4>
              <ul className="space-y-2">{["Ekonomi","Politik","Teknologi","Bisnis","Keuangan","Infografis"].map((item) => (<li key={item}><Link href={`/?category=${item}`} className="text-xs text-gray-500 hover:text-red-700 transition">{item}</Link></li>))}</ul>
            </div>
            <div>
              <h4 className="font-bold text-gray-800 mb-4 text-sm">Layanan</h4>
              <ul className="space-y-2">{["Newsletter","RSS Feed","Pengumuman","Laporan","Iklan","Press Release"].map((item) => (<li key={item}><a href="#" className="text-xs text-gray-500 hover:text-red-700 transition">{item}</a></li>))}</ul>
            </div>
            <div>
              <h4 className="font-bold text-gray-800 mb-4 text-sm">Informasi</h4>
              <ul className="space-y-2">{["Tentang Kami","Redaksi","Pedoman Media Siber","Karir","Disclaimer","Kebijakan Privasi"].map((item) => (<li key={item}><a href="#" className="text-xs text-gray-500 hover:text-red-700 transition">{item}</a></li>))}</ul>
            </div>
            <div>
              <h4 className="font-bold text-gray-800 mb-4 text-sm">Jaringan Media</h4>
              <ul className="space-y-2">{["CNN Indonesia","CNBC Indonesia","Detik Network","Kompas","Tempo","Bisnis.com"].map((item) => (<li key={item}><a href="#" className="text-xs text-gray-500 hover:text-red-700 transition">{item}</a></li>))}</ul>
            </div>
          </div>
          <div className="mt-10 pt-6 border-t border-gray-200 flex flex-col md:flex-row items-center justify-between gap-2">
            ...
            <p className="text-xs text-gray-400">Konten bersifat informatif, untuk kepentingan informasi.</p>
          </div>
        </div>
      </footer>
    </main>
