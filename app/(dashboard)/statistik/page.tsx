...
// Contoh: +2 views, -1 artikel, +5s
function formatDelta(delta: number | null, unit: string = ""): string | null {
  ...
  if (active && payload?.length) {
    return (
      <div className="bg-white/98 backdrop-blur border border-indigo-100 rounded-xl px-4 py-3 shadow-xl text-sm">
        <p className="font-medium text-zinc-500 text-xs mb-1">
...
const CustomYAxisTick = (props: {
  ...
  return (
    <g transform={`translate(${x},${y})`}>
      <text x={0} y={0} dy={4} textAnchor="end" fill="#71717a" fontSize={10} fontFamily="inherit">
        {display}
      </text>
    </g>
  );
...
function StatCard({
    ...
    return (
      <div className="group rounded-2xl p-5 border border-zinc-100 bg-white hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300 cursor-default">
        <div className="flex items-start justify-between mb-3">
          <span className={`w-9 h-9 rounded-xl ${flatTheme.icon} flex items-center justify-center ${flatTheme.text}`}>
            {icon}
          </span>
  ...
  return (
    <div
      className={`group rounded-2xl p-5 bg-gradient-to-br ${gradient} hover:shadow-xl hover:-translate-y-1 transition-all duration-300 cursor-default relative overflow-hidden`}
    >
    ...
    <div className="group flex items-center gap-3 py-3 px-3 -mx-3 rounded-xl hover:bg-zinc-50 transition-all duration-200 cursor-default">
      <span
        className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold shrink-0 shadow-sm ${
          RANK_COLORS[rank - 1] ?? RANK_COLORS[4]
        }`}
      >
        {rank}
      </span>
      <div className="flex-1 min-w-0">
        <p className="text-sm font-medium text-zinc-800 truncate leading-tight group-hover:text-zinc-900 transition-colors">
          {article.title}
        </p>
        <div className="flex items-center gap-2 mt-1">
          <span className={`text-[10px] font-medium px-2 py-0.5 rounded-full ${colorDot} capitalize`}>
            {article.category}
          </span>
  ...
  return (
    <div className="space-y-2 mt-1">
      {data.map((item, i) => (
        <div
          key={item.name}
          className="group flex items-center gap-2.5 hover:bg-zinc-50 rounded-lg px-2 py-1 -mx-2 transition-colors cursor-default"
        >
          <span
            className="w-3 h-3 rounded-full shrink-0 shadow-sm"
            style={{ backgroundColor: PIE_COLORS[i % PIE_COLORS.length] }}
          />
          <span className="text-xs text-zinc-600 capitalize flex-1 truncate group-hover:text-zinc-900 transition-colors font-medium">
            {item.name}
          </span>
          <div className="flex items-center gap-2 shrink-0">
            ...
            <span
              className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full ${
                CATEGORY_BADGE_COLORS[i % CATEGORY_BADGE_COLORS.length]
              }`}
            >
...
const GradientBar = (props: {
  ...
  const colors = [
    "#6366f1","#22d3ee","#f59e0b","#10b981",
    "#f43f5e","#a78bfa","#fb923c","#34d399","#6366f1","#22d3ee",
  ];
...
const CustomLineDot = (props: {
  ...
  const { cx = 0, cy = 0, value = 0, index = 0, dataLength = 0 } = props;
  if (value === 0) return null;
  // Hanya tampilkan dot di titik pertama, terakhir, dan nilai tertinggi
  const isEdge = index === 0 || index === dataLength - 1;
  if (!isEdge && value < 2) return null;
  return (
    <circle
      cx={cx}
      cy={cy}
...
function AuthorRow({
  ...
  return (
    <div className="group flex items-center gap-3 py-3 px-3 -mx-3 rounded-xl hover:bg-zinc-50 transition-all duration-200 cursor-default">
      {/* Rank */}
      <span
        className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold shrink-0 shadow-sm ${rankTheme.badge}`}
      >
        {rank}
      </span>
      ...
      <div
        ...
        style={{ backgroundColor: rankTheme.bar }}
      >
        {initials}
      </div>
      ...
      <div className="text-right shrink-0 space-y-0.5">
        <p className="text-sm font-bold text-zinc-900 tabular-nums">
          {author.views.toLocaleString("id-ID")}
        </p>
...
export default function StatistikPage() {
  ...
  const fetchStats = useCallback(
    async (opts: { isRefresh?: boolean; newPeriod?: Period } = {}) => {
      ...
      try {
        const res = await fetch(`/api/statistics?period=${activePeriod}`, {
          credentials: "include",
        });
 ...
 const {
  ...
  // Hitung max views untuk trend chart referensi
  const maxTrendViews = Math.max(...trendData.map((d) => d.views), 1);
  const avgTrendViews = Math.round(
    trendData.reduce((s, d) => s + d.views, 0) / (trendData.length || 1)
  );
  ...
  const row1Cards = [
    ...
    },
    {
      ...
      sub: isAllTime
        ? `${articlesByCategory.length} kategori aktif`
        : `dari ${summary.publishedArticles} total artikel`,
      delta: isAllTime ? null : summary.delta.publishedArticles,
      deltaUnit: " artikel",
      icon: (
        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
    ...
    {
      label: "Artikel Draft",
      value: summary.draftArticles,
      gradient: CARD_THEMES[2].bg,
      sub: "Menunggu publish",
      delta: null,
      deltaUnit: "",
      icon: (
        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    {
      label: "Growth Minggu Ini",
      value: `${growthPositive ? "+" : ""}${summary.growth}%`,
  ...
  const row2Cards = [
    {
      ...
      theme: CARD_THEMES[4],
      delta: isAllTime ? null : summary.delta.views,
      deltaUnit: " views",
      icon: (
        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
    ...
    },
    {
      ...
      sub: isAllTime ? "Pengunjung unik" : PERIOD_LABELS[period],
      theme: CARD_THEMES[6],
      delta: isAllTime ? null : summary.delta.uniqueVisitors,
      deltaUnit: " visitor",
      icon: (
        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
    ...
    {
      label: "Avg. Waktu Baca",
      value: summary.avgTimeRead,
      sub: isAllTime ? "Rata-rata per sesi" : PERIOD_LABELS[period],
      theme: CARD_THEMES[7],
      delta: isAllTime ? null : summary.delta.avgTimeRead,
      deltaUnit: "s",
      icon: (
        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
  ...
  // Judul label axis trend
  const trendXInterval = (() => {
    if (period === "today") return 3;     // setiap 3 jam
    if (period === "7d") return 1;
    if (period === "30d") return 4;       // setiap 4 hari
    if (period === "90d") return 2;
    return 2;                              // monthly
  })();
    ...
    <div className={`space-y-6 transition-opacity duration-200 ${isTransitioning ? "opacity-60" : "opacity-100"}`}>
      ...
      {!isAllTime && (
        <div className="flex items-center gap-2 px-4 py-2.5 bg-indigo-50 border border-indigo-100 rounded-xl">
          ...
          <p className="text-xs text-indigo-600 font-medium">
            ...
            <span className="font-bold">{PERIOD_LABELS[period]}</span>
            {" · "}Badge menunjukkan <span className="font-bold">perubahan absolut</span> vs periode sebelumnya (misal: +2 views, -1 artikel)
          </p>
        </div>
      )}
            ...
            <div className="flex items-center gap-3 px-3 py-1.5 bg-zinc-50 rounded-xl">
              <div className="text-center">
                ...
                <p className="text-xs font-bold text-indigo-600">
                  {maxTrendViews.toLocaleString("id-ID")}
                </p>
              </div>
          ...
          <div className="flex flex-col items-center justify-center h-48 text-zinc-300 gap-2">
            ...
            <p className="text-xs text-zinc-400">
              {isAllTime
                ...
                : `Tidak ada views di ${PERIOD_LABELS[period]}`}
            </p>
          </div>
        ) : (
        ...
        <div className="bg-white rounded-2xl border border-zinc-100 p-6 shadow-sm hover:shadow-md transition-shadow duration-300">
          <div className="flex items-center justify-between mb-5">
            <div>
              ...
              <p className="text-xs text-zinc-400 mt-0.5">
                {pieTotal} artikel {isAllTime ? "publish" : `di ${PERIOD_LABELS[period]}`}
              </p>
            </div>
            <span className="px-2.5 py-1 bg-indigo-50 text-indigo-600 text-xs font-semibold rounded-lg">
              {articlesByCategory.length} kategori
            </span>
          </div>
          ...
          {articlesByCategory.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-48 text-zinc-300 gap-2">
              ...
              <p className="text-xs text-zinc-400">
                {isAllTime ? "Belum ada data kategori" : `Tidak ada artikel di ${PERIOD_LABELS[period]}`}
              </p>
            </div>
          ) : (
        ...
        <div className="bg-white rounded-2xl border border-zinc-100 p-6 shadow-sm hover:shadow-md transition-shadow duration-300">
          <div className="flex items-center justify-between mb-5">
            <div>
              ...
              <p className="text-xs text-zinc-400 mt-0.5">
                Top 10 · {isAllTime ? "all time" : PERIOD_LABELS[period]}
              </p>
            </div>
            <span className="px-2.5 py-1 bg-cyan-50 text-cyan-600 text-xs font-semibold rounded-lg">
              Top 10
            </span>
          </div>
          ...
          {viewsPerArticle.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-48 text-zinc-300 gap-2">
              ...
              <p className="text-xs text-zinc-400">
                {isAllTime ? "Belum ada data views" : `Tidak ada views di ${PERIOD_LABELS[period]}`}
              </p>
            </div>
          ) : (
            <ResponsiveContainer width="100%" height={210}>
              <BarChart
                data={viewsPerArticle}
                layout="vertical"
        ...
        <div className="bg-white rounded-2xl border border-zinc-100 p-6 shadow-sm hover:shadow-md transition-shadow duration-300">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-xs text-zinc-400 mt-0.5">
                Paling banyak dibaca {isAllTime ? "" : `· ${PERIOD_LABELS[period]}`}
              </p>
            </div>
          ...
          {topArticles.length === 0 ? (
            <p className="text-xs text-zinc-400 py-8 text-center">
              {isAllTime ? "Belum ada artikel yang dipublish" : `Tidak ada views di ${PERIOD_LABELS[period]}`}
            </p>
          ) : (
            topArticles.map((article, i) => (
              <ArticleRow
                ...
                article={article}
                variant="top"
                colorDot={CATEGORY_BADGE_COLORS[i % CATEGORY_BADGE_COLORS.length]}
              />
            ))
          )}
        </div>
        ...
        <div className="bg-white rounded-2xl border border-zinc-100 p-6 shadow-sm hover:shadow-md transition-shadow duration-300">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-xs text-zinc-400 mt-0.5">
                Perlu perhatian lebih {isAllTime ? "" : `· ${PERIOD_LABELS[period]}`}
              </p>
            </div>
          ...
          {bottomArticles.length === 0 ? (
            <p className="text-xs text-zinc-400 py-8 text-center">
              {isAllTime ? "Belum ada artikel yang dipublish" : `Tidak ada data di ${PERIOD_LABELS[period]}`}
            </p>
          ) : (
      ...
      <div className="bg-white rounded-2xl border border-zinc-100 p-6 shadow-sm hover:shadow-md transition-shadow duration-300">
        <div className="flex items-center justify-between mb-5">
          <div>
            ...
            <p className="text-xs text-zinc-400 mt-0.5">
              ...
              {!isAllTime && (
                <span className="text-indigo-500 font-medium"> · {PERIOD_LABELS[period]}</span>
              )}
            </p>
          </div>
          <div className="flex items-center gap-2">
            <span className="px-2.5 py-1 bg-violet-50 text-violet-600 text-xs font-semibold rounded-lg">
              {authorPerformance.length} penulis
            </span>
          </div>
        </div>
        ...
        {authorPerformance.length === 0 ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-x-8">
            ...
            <div>
              {authorPerformance.slice(0, Math.ceil(authorPerformance.length / 2)).map((author, i) => (
                <AuthorRow
                  ...
                  rank={i + 1}
                  author={author}
                  maxViews={authorPerformance[0]?.views || 1}
                />
              ))}
            </div>
            {/* Kolom kanan */}
            {authorPerformance.length > 1 && (
              <div>
                {authorPerformance
                  .slice(Math.ceil(authorPerformance.length / 2))
                  .map((author, i) => (
                    <AuthorRow
                      key={author.authorId}
                      rank={Math.ceil(authorPerformance.length / 2) + i + 1}
                      author={author}
                      maxViews={authorPerformance[0]?.views || 1}
                    />
                  ))}
        ...
        {authorPerformance.length > 0 && (
          <div className="mt-4 pt-4 border-t border-zinc-50 flex items-center gap-2">
            ...
            <p className="text-[10px] text-zinc-400">
              Progress bar menunjukkan proporsi views relatif terhadap penulis teratas
              {!isAllTime && ` · data ${PERIOD_LABELS[period]}`}
            </p>
          </div>
        )}
      </div>
      ...
      <div className="flex items-center justify-center gap-2 pb-2">
        ...
        <p className="text-[11px] text-zinc-400">
          Data diperbarui secara real-time · Unique visitors & waktu baca aktif setelah PageView tracking terpasang
        </p>
      </div>
